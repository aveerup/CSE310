...
class SymbolInfo{
    ...
    void set_scope(double scope){
        this->scope = scope;
    }

    void set_info_no(int info_no){
        this->info_no = info_no;
    }

    void set_total_parameter(int total_parameter){
        this->total_parameter = total_parameter;
    }

    void set_is_argument(bool is_argument){
        this->is_argument = is_argument;
    }

    void set_variable_offset(int variable_offset){
        this->variable_offset = variable_offset;
    }
    
    void set_variable_size(int variable_size){
    ...
    // return the scope id in which symbolinfo is
    double get_scope(){
        return this->scope;
    }

    // the place of a local variable among other ones
    int get_info_no(){
        return this->info_no;
    }

    // for storing number of parameters if the symbolInfo 
    // is a function
    int get_total_parameter(){
        return this->total_parameter;
    }

    // checks if a symbolinfo is argument to a function
    bool get_is_argument(){
        return this->is_argument;
    }

    int get_variable_offset(){
        return this->variable_offset;
    }
...
class ScopeTable{
    ...
    void set_initial_var_offset(int initial_variable_offset){
        this->initial_variable_offset = initial_variable_offset;
    }
    ...
    int get_initial_var_offset(){
        return this->initial_variable_offset;
    }
    ...
    bool   Insert(SymbolInfo *symbol){
        ...
        if(exists == NULL){
            ...
            symbol->set_scope(this->id);
            if(!symbol->get_is_func() && !symbol->get_is_array()){
                this->variable_count++;
                cout<< "1.11 " << this->variable_count <<endl;
                symbol->set_info_no(variable_count);
            }
...
class SymbolTable{
    ...
    double get_curr_scope_id(){
        return this->scopeTables->get_id();
    }

    int get_curr_scope_variable_no(){
        return scopeTables->get_variable_count();
    }

    void set_curr_parameter_count(int parameter_count){
        this->scopeTables->set_parameter_count(parameter_count);
    }

    int get_curr_parameter_count(){
        return this->scopeTables->get_parameter_count();
    }

    void set_initial_var_offset(int initial_var_offset){
        this->scopeTables->set_initial_var_offset(initial_var_offset);
    }

    int get_initial_var_offset(){
        return this->scopeTables->get_initial_var_offset();
    }
